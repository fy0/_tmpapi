export default {
    accounts: {
      ...
      openai: {
        ...
        turnStateAutoDesc:
          'When enabled, the system takes over: once a session is seen at 312, the most recent valid 292 for this account and model is injected. If the upstream still mints 312 after injection, that candidate is marked failed and the next one is used; when every candidate for that model fails the account is disabled with the reason recorded. Candidates are bucketed per account and model (a turn-state does not carry across models) and are valid for 4 minutes from minting. A hunted 292 is sent with routing cookies for that account (__cflb / __oailb). Those cookies are not bound to a ticket and expire in about 2 minutes; once they expire the hunter mints again. Once the ticket expires nothing is injected and the system waits for a fresh 292. The manual value above stops taking effect. HTTP paths only — WebSocket passthrough is not covered.',
        ...
        cookieLock: 'Cookie lock',
        cookieLockDesc: 'Lock a pod observed serving the requested model independently of state takeover. Disable state takeover and clear manual state to use cookies alone. Per-account pool, HTTP only; expiry or model mismatch replaces the candidate. An empty pool passes through unless degraded scheduling pause is enabled (failover or 503).',
        cookieHunterDesc: 'Sample distinct pods through selected proxies. A matching response.created model is a hit; collect up to 3 healthy pods. Try renewing with the same cookie at least 2 minutes before expiry, within existing hourly, interval and idle limits. Pod health follows observations; no seven-day exit cooldown.',
        cookiePoolEmpty: 'Cookie lock: no usable candidates',
        cookiePoolSummary: 'Cookie lock: {n} usable candidates',
        cookiePoolItem: '{pod} · requested {model} → served {served} · expires {expiry} · {status}',
        cookieReady: 'ready',
        cookieUnavailable: 'failed or expired',
        cookieHuntResult: '{status} · {pod} → {model}',
        turnStateHunter: 'Ticket hunter',
        turnStateHunterDesc:
        ...
        turnStateHunterNeedsAuto: 'Enable state takeover or Cookie lock first, otherwise the hunter does not run',
        ...
        turnStateHunterHoldDesc:
          'When a hunted model has no usable state or cookie, pause that model on this account for one idle window (idle_minutes) and fail the request over (503 if no other account); after expiry the next request re-pauses it if still no ticket, and a new ticket resumes it immediately. While this is on, a live ticket is also voided when the actual response model detaches from the requested model. Other models are unaffected; a model nobody requests anymore simply expires.',
        ...
        turnStatePool: {
          ...
          hunterNeedsAuto: 'hunter inactive: enable state takeover or Cookie lock first',
